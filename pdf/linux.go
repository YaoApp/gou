...
// LinuxDriver implements Command for Linux systems
type LinuxDriver struct {
	toolPaths ToolPaths
}

// NewLinuxDriver creates a new Linux driver with optional custom tool paths
func NewLinuxDriver(customPaths ToolPaths) *LinuxDriver {
	// Default tool paths for Linux
	defaultPaths := ToolPaths{
		ToolPdftoppm:    "pdftoppm", // apt install poppler-utils
		ToolMutool:      "mutool",   // apt install mupdf-tools
		ToolImageMagick: "convert",  // apt install imagemagick
	}
...
// IsAvailable checks if a specific tool is available on Linux
func (d *LinuxDriver) IsAvailable(tool ConvertTool) bool {
...
// GetToolPath returns the executable path for a specific tool on Linux
func (d *LinuxDriver) GetToolPath(tool ConvertTool) string {
...
// ConvertWithPdftoppm converts PDF to images using pdftoppm on Linux
func (d *LinuxDriver) ConvertWithPdftoppm(ctx context.Context, filePath string, config ConvertConfig, pageCount int) ([]string, error) {
	return d.executePdftoppm(ctx, filePath, config, pageCount)
}

// ConvertWithMutool converts PDF to images using mutool on Linux
func (d *LinuxDriver) ConvertWithMutool(ctx context.Context, filePath string, config ConvertConfig, pageCount int) ([]string, error) {
	return d.executeMutool(ctx, filePath, config, pageCount)
}

// ConvertWithImageMagick converts PDF to images using ImageMagick on Linux
func (d *LinuxDriver) ConvertWithImageMagick(ctx context.Context, filePath string, config ConvertConfig, pageCount int) ([]string, error) {
	return d.executeImageMagick(ctx, filePath, config, pageCount)
}

// executePdftoppm executes pdftoppm command on Linux
func (d *LinuxDriver) executePdftoppm(ctx context.Context, filePath string, config ConvertConfig, pageCount int) ([]string, error) {
...
// executeMutool executes mutool command on Linux
func (d *LinuxDriver) executeMutool(ctx context.Context, filePath string, config ConvertConfig, pageCount int) ([]string, error) {
...
// executeImageMagick executes ImageMagick convert command on Linux
func (d *LinuxDriver) executeImageMagick(ctx context.Context, filePath string, config ConvertConfig, pageCount int) ([]string, error) {
...
func (d *LinuxDriver) parsePageRange(pageRange string, pageCount int) (int, int) {
...
// buildOutputPaths builds the list of output file paths
func (d *LinuxDriver) buildOutputPaths(config ConvertConfig, startPage, endPage int) ([]string, error) {
	var paths []string
...
func (d *LinuxDriver) buildOutputPathsImageMagick(config ConvertConfig, startPage, endPage int) ([]string, error) {
	var paths []string
