...
TESTFOLDER := $(shell $(GO) list ./... | grep -E 'server/http|process|widget|model|schema|lang|query|task|schedule|flow|session|store|fs|http|encoding|ssl|plugin|connector|wasm|websocket$|v8' | grep -v -E 'wamr|socket')
TESTTAGS ?= ""
