...
type GouQueryDSL struct {
	...
	Unions   []GouQueryDSL `json:"unions,omitempty"`    // 联合查询
	Query    *GouQueryDSL  `json:"query,omitempty"`     // 子查询
	Joins    []Join        `json:"joins,omitempty"`     // 表连接
	SQL      SQL           `json:"sql,omitempty"`       // SQL语句
	Comment  string        `json:"comment,omitempty"`   // 查询条件注释
}

// GouQueryDSLSugar ?支持语法糖
type GouQueryDSLSugar struct {
	GouQueryDSL
...
type Where struct {
	...
	Query   GouQueryDSL `json:"query,omitempty"`   // 子查询, 如设定 query 则忽略 value 数值。
	Comment string      `json:"comment,omitempty"` // 查询条件注释
}
