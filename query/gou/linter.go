...
type LintResult struct {
	DSL         *QueryDSL    `json:"dsl,omitempty"` // Parsed DSL (nil if parsing failed)
...
// Returns: (*QueryDSL, *LintResult) - DSL is nil if parsing/validation failed
func Parse(source string) (*QueryDSL, *LintResult) {
	result := &LintResult{
		Source:      source,
	...
	// Try to parse JSON
	var dsl QueryDSL
	if err := json.Unmarshal([]byte(source), &dsl); err != nil {
...
func MustParse(source string) *QueryDSL {
...
func (r *LintResult) validateWhere(where Where, path string, source string) {
...
func (r *LintResult) validateHaving(having Having, path string, source string) {
