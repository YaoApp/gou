...
func (ws WebSocket) onConnected(option websocket.WSClientOption) error {
	...
	p, err := ProcessOf(ws.Event.Connected, option)
	if err != nil {
		return err
	}
...
func (ws WebSocket) onClosed(data []byte, err error) []byte {
	...
	p, err := ProcessOf(ws.Event.Closed, msg, errstr)
	if err != nil {
...
func (ws WebSocket) onData(data []byte, recvLen int) ([]byte, error) {
	...
	p, err := ProcessOf(ws.Event.Data, string(data), recvLen)
	if err != nil {
		return nil, err
	}
	res, err := p.Exec()
...
func (ws WebSocket) onError(err error) {
	...
	p, err := ProcessOf(ws.Event.Error, err.Error())
	if err != nil {
