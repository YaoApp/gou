...
// 	res := process.New("plugins.user.Login", 1).Run().(maps.Map)
// 	res2 := process.New("plugins.user.Login", 2).Run().(maps.Map)
...
// 	res := process.New("scripts.app.test.hello", "world").Run()
// 	assert.Equal(t, "hello:world", res)

// 	res = process.New("scripts.app.test.helloProcess", "Max").Run()
...
// 	res = process.New("scripts.flows.script.rank.hello").Run()
// 	assert.Equal(t, "rank hello", res)
// }

// func TestProcessFlow(t *testing.T) {
// 	res := maps.Of(process.New("flows.latest", "%公司%", "bar").Run().(map[string]interface{}))
...
// 	res := process.New("models.user.Find", 1, QueryParam{}).Run().(maps.MapStr)
// 	assert.Equal(t, 1, any.Of(res.Dot().Get("id")).CInt())
// 	assert.Equal(t, "男", res.Dot().Get("extra.sex"))
// }

// func TestProcessGet(t *testing.T) {
// 	rows := process.New("models.user.Get", QueryParam{Limit: 2}).Run().([]maps.MapStr)
...
// 	rows := process.New("models.user.SelectOption").Run().([]maps.MapStr)
// 	res := maps.Map{"data": rows}.Dot()
...
// 	rows = process.New("models.user.SelectOption", "用户").Run().([]maps.MapStr)
// 	res = maps.Map{"data": rows}.Dot()
// 	assert.Equal(t, 1, len(rows))
// 	assert.Equal(t, 3, any.Of(res.Get("data.0.id")).CInt())
// 	assert.Equal(t, "用户", res.Get("data.0.name"))

// 	rows = process.New("models.user.SelectOption", "", "name", "name").Run().([]maps.MapStr)
// 	res = maps.Map{"data": rows}.Dot()
// 	assert.Equal(t, 3, len(rows))
...
// 	res := process.New("models.user.Paginate", QueryParam{}, 1, 2).Run().(maps.MapStr).Dot()
...
// 	}
// 	id := process.New("models.user.Create", row).Run().(int)
...
// 	id := process.New("models.user.Update", 1, maps.MapStr{"balance": 200}).Run()
// 	assert.Nil(t, id)
...
// 		"extra":    maps.MapStr{"sex": "女"},
// 	}
// 	id := process.New("models.user.Save", row).Run()
...
// 	process.New("models.user.Delete", id).Run()
...
// 	user := Select("user")
// 	id := user.MustSave(row)
// 	process.New("models.user.Destroy", id).Run()
...
// 	process.New("models.address.Insert", payload["columns"], payload["rows"]).Run()
...
// 	effect := process.New("models.user.UpdateWhere",
// 		QueryParam{
// 			Limit: 1,
// 			Wheres: []QueryWhere{
// 				{
...
// 		},
// 	}}
// 	effect := process.New("models.user.DeleteWhere", param).Run().(int)
...
// 		},
// 	}}
// 	effect := process.New("models.user.DestroyWhere", param).Run().(int)

// 	// 清理数据
// 	assert.Equal(t, effect, 3)
// }

// func TestProcessRegisterprocess.Handler(t *testing.T) {

// 	Registerprocess.Handler("charts", func(process *process.Process) interface{} {
// 		return maps.Map{
// 			"name":   "charts",
// 			"class":  process.ID,
...
// 	Registerprocess.Handler("charts.user.world", func(process *process.Process) interface{} {
// 		return maps.Map{
...
// 	res := process.New("charts.user.Hello", "foo", "bar").Run().(maps.Map)
...
// 	res2 := process.New("charts.user.World", "bar", "foo").Run().(maps.Map)
...
// 	process := process.New("session.Set", "foo", "bar").WithSID(id).WithGlobal(global)
// 	process.Run()

// 	process = process.New("session.Get", "foo").WithSID(id).WithGlobal(global)
// 	foo := process.Run()
// 	assert.Equal(t, "bar", foo)

// 	process = process.New("session.Dump").WithSID(id).WithGlobal(global)
// 	data := process.Run()
...
// 	global := map[string]interface{}{"hello": "world"}
// 	process := process.New("session.Set", "foo", "bar", 3600, id).WithGlobal(global)
// 	process.Run()

// 	process = process.New("session.Get", "foo", id).WithGlobal(global)
// 	foo := process.Run()
// 	assert.Equal(t, "bar", foo)

// 	process = process.New("session.Dump", id).WithGlobal(global)
// 	data := process.Run()
// 	assert.Equal(t, map[string]interface{}{"foo": "bar"}, data)
// }
...
// 	process := process.New("models.user.EachSave", args...)
// 	res := process.Run()
...
// 	process := process.New("models.user.EachSaveAfterDelete", args...)
// 	res := process.Run()
...
// 	value, err := process.New("models.user.Find", 1, QueryParam{}).Exec()
// 	res := value.(maps.MapStr)
// 	assert.Nil(t, err)
// 	assert.Equal(t, 1, any.Of(res.Dot().Get("id")).CInt())
// 	assert.Equal(t, "男", res.Dot().Get("extra.sex"))

// 	value, err = process.New("models.user.Find", 100, QueryParam{}).Exec()
// 	assert.Nil(t, value)
...
// 	lang := process.New("models.user.Find", 1, QueryParam{}).Lang()
// 	assert.Equal(t, "", lang)

// 	sid := session.ID()
// 	session.Global().ID(sid).Set("__yao_lang", "zh-cn")

// 	lang = process.New("models.user.Find", 1, QueryParam{}).WithSID(sid).Lang()
